...
namespace chefstock_platform.RestaurantManagement.Interfaces.ACL
{
    public interface IEmployeeContextFacade
    {
        ...
        Task<Employee?> FetchEmployeeById(int id);
        Task<IEnumerable<Employee>> FetchAllEmployees();
        Task UpdateEmployee(int id, string firstName, string lastName, string email, string phone, string position, int restaurantId);
